...
class Parser
{
	...
	public:
		static Users&			get_users(void);
		static Channels&		get_channels(void);
		Parser(Udata& serv_udata);
		~Parser();
		void					command_parser(const uintptr_t& ident, std::string &command);
};

Users&		Parser::get_users(void)
{
	static Users	users_;
	return users_;
}

Channels&	Parser::get_channels(void)
{
	static Channels	channels_;
	return channels_;
}
...
KeventHandler	Parser::get_Kevent_Handler(void)
{
	return kq_;
}
...
Parser::Parser(Udata& serv_udata) : parser_udata_(serv_udata) {
}
...
void	Parser::push_multiple_write_events_(Udata& ret, const uintptr_t& ident)
{
	...
	for (Event_iter iter = ret.begin(); iter != ret.end(); ++iter)
	{
		...
		get_Kevent_Handler().set_write(iter->first);
	}
}
