...
// 	my_user = search_user_by_ ident(event.ident);
// 	std::cout << my_user->event.ident << std::endl;
// 	if (pair.second) // 중복 O
// 	{
// 		std::cout << "is dup??" << std::endl;
// 		//std::cout << "what the fuck> " << (*my_user).evunt.ident << std::endl;
// 		return (Sender::nick_error_message(*pair.first, nickname));
